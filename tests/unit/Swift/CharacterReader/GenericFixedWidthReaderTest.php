  ...
  extends UnitTestCase
{
  ...
  public function testValidationValueIsBasedOnOctetCount()
  {
    ...
    $this->assertIdentical(
      1, $reader->validateByteSequence(array(0x01, 0x02, 0x03))
      ); //3 octets
    
    $this->assertIdentical(
      2, $reader->validateByteSequence(array(0x01, 0x0A))
      ); //2 octets
    
    $this->assertIdentical(
      3, $reader->validateByteSequence(array(0xFE))
      ); //1 octet
    
    $this->assertIdentical(
      0, $reader->validateByteSequence(array(0xFE, 0x03, 0x67, 0x9A))
      ); //All 4 octets
  }
  ...
  public function testValidationFailsIfTooManyOctets()
  {
    ...
    $this->assertIdentical(-1, $reader->validateByteSequence(
      array(0xFE, 0x03, 0x67, 0x9A, 0x10, 0x09, 0x85)
      )); //7 octets
  }
